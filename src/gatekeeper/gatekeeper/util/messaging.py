"""
Utilities for pishahang-internal messaging – heavily based on messaging.py in mano-base
"""
...
BROKER_EXCHANGE = config.broker.exchange
...
class ManoBrokerConnection:
    ...
    def subscribe(self, cbf, topic, subscription_queue=None):
        ...
        Starts a new thread for each subscription in which messages are consumed and the callback
        ...
        # Attention: We crate an individual queue for each subscription to allow multiple
...
class ManoBrokerRequestResponseConnection(ManoBrokerConnection):
    ...
    def call_async(
        ...
        reply and calls it when the reply is received.
        ...
        :param correlation_id: used to match requests to replies. If correlation_id is not given, a
...
class ConnexionBrokerConnection(ManoBrokerRequestResponseConnection):
    """
    A subclass of `ManoBrokerRequestResponseConnection` that adds convenient methods for synchronous
    calls within Connexion endpoint handler functions.
    """
    ...
    def call_sync_simple(self, *args, **kwargs):
        ...
        if "msg" in kwargs:
            ...
            kwargs["content_type"] = "application/x-json"
