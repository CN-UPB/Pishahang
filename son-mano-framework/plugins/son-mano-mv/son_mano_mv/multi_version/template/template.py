class Template:
    ...
    def weight(self, src_dr):
        ...
        for j in topo_order:
            ...
            # forward direction refers to the ingoing data rates and includes end components
            else:                # ingoing data rates in forward direction
                in_dr_fwd = []
                for k_in in range(j.inputs):
                    # get ingoing arc at k_in
                    in_arcs = [a for a in self.arcs if a.dest == j and a.dest_in == k_in]
                    # if the component is adapted on the fly, k_in might belong to another template => set dr to 0
                    if len(in_arcs) == 0:
                        in_dr_fwd.append(0)
                    elif len(in_arcs) == 1:
                        in_arc = in_arcs[0]
                        in_dr_fwd.append(out_dr[(in_arc.source, in_arc.src_out)])
                    else:
                ...
                cpu = j.cpu_req(in_dr_fwd)
                gpu = j.gpu_req(in_dr_fwd)
                total_cpu += cpu
                total_gpu += gpu
            ...
            #     mem = j.gpu_req(in_dr_fwd + in_dr_bwd)
            #     total_cpu += cpu
            #     total_mem += mem
            #     total_gpu += gpu
    ...
    # start with source component and continue breadth-first style
    ...
    def topological_component_order(self):
        # fwd_order stores all ordered components 
        ...
        while len(curr_level) > 0:
            next_level = []
            for j in curr_level:
                fwd_arcs_out = [a for a in self.arcs if a.source == j]
