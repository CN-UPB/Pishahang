...
def objective_value(overlays, print_info=False):
	...
	consumed_cpu, consumed_gpu = consumed_node_resources(overlays)
	...
	total_consumed_gpu = sum(consumed_gpu[v] for v in nodes.ids)
	...
	# if print_info:
		...
		##print("Total consumed resources: {} (cpu), {} (gpu), {} (dr)".format(total_consumed_cpu, total_consumed_gpu, total_consumed_dr))
		...
		#logging.info("Total consumed resources: {} (cpu), {} (gpu), {} (dr)".format(total_consumed_cpu, total_consumed_gpu, total_consumed_dr))
	...
	if obj == objective.COMBINED:
		...
		value += w3 * (total_consumed_cpu + total_consumed_gpu + total_consumed_dr)
		value += total_delay
	...
	# minimize total resource consumption
		...
		value = total_consumed_cpu + total_consumed_gpu + total_consumed_dr
...
def solve(arg_nodes, arg_links, templates, prev_overlays, sources, fixed, arg_obj):
	...
	prev_instances = {Instance(i.component, i.location, version=i.version, src_dr=i.src_dr) for ol in prev_overlays.values() for i in ol.instances}
	obj = arg_obj
	...
	# if len(nodes.ids) > 1:		# doesn't work for networks with just 1 node
		# ##print("\n----- SKIPPED Iterative improvement -----")
		# #logging.info("----- SKIPPED Iterative improvement -----")
