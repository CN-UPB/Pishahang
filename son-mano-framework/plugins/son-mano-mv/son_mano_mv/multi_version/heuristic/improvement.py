...
# reset overlay of specified template and update linked_stateful in the overlays_to_modify
# keep instances before the specified instance (less changes, shorter runtime)
def reset_overlay(template, instance, overlays_to_modify):
	...
	# reset linked_stateful in all instances of all overlays
	# all_instances = [i for ol in overlays_to_modify.values() for i in ol.instances]
	# for i in all_instances:
	# 	i.reset_linked()

	# set linked_stateful by iterating through each overlay in topological order
	for ol in overlays_to_modify.values():
		direction = "forward"
		...
		for i in ol.topological_order():
			if i.component.end:
				end_reached = True
			# as all end instances come in a row (no other in between), it is correct to switch after the last one
			if end_reached and not i.component.end:		# switch direction after last end instance
				direction = "backward"
			i.update_linked(direction)
