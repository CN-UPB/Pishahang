...
echo "Starting son-broker.."

sudo docker stop son-broker
sudo docker rm son-broker
# sudo docker build -t son-sp-infrabstract -f Dockerfile-dev .
sudo docker run -d --name son-broker --net=son-sp --network-alias=son-broker -p 5672:5672 rabbitmq:3.6.15-management
