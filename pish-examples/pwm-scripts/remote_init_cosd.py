"""
A python script that can be used to instantiate a COSD in Pishahang remotely.
Requirements:
- Python3.6
...
pishahang_cloud = wrappers.SONATAClient.Pishahang(HOST_URL)
...
# Upload VNF
VNFD_PATH = "{desc}/{name}_csd.yml".format(desc=DESCRIPTORS_PATH, name=VNFDNAME)
_res = pishahang_cloud.post_csd_descriptors(token=token["token"]["access_token"],
                                            package_path=VNFD_PATH)
print(_res)
time.sleep(0.5)

# Upload NSD
NSD_PATH = "{desc}/{name}_cosd.yml".format(desc=DESCRIPTORS_PATH, name=NSDNAME)
_res = pishahang_cloud.post_cosd_descriptors(token=token["token"]["access_token"],
                                            package_path=NSD_PATH)
print(_res)
time.sleep(0.5)

# Get NSD UUID and init
_nsd_list = json.loads(pishahang_cloud.get_cosd_descriptors(token=token["token"]["access_token"], limit=1000))
_nsd_list = json.loads(_nsd_list["data"])

_ns = None
for _n in _nsd_list:
    if NSDNAME == _n['cosd']['name']:            
        _ns = _n['uuid']
        print("UUID")
        print(_ns)
        continue
...
if _ns:
    ...
    instantiation = json.loads(pishahang_cloud.post_cs_instances_nsinstanceid_instantiate(
