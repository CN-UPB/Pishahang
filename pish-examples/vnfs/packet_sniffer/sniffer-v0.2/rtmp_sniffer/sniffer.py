...
class RTMPSniffer(object):
    ...
    def __init__(self):
        ...
        if 'CTR_IPA' in os.environ:
            self.ctr_ip = os.environ['CTR_IPA']
        else:
        ...
        while True:
            try:
                ...
                LOG.error ("Connecting to RabbitMQ failed, will try one more time in 5 seconds!")
                time.sleep(5)
    ...
    def update_ctr(self, ctr_ip):
       try:
           ...
           LOG.error("Passing IP to SDN controller failed! ERROR: {0}".format(str(err)))
