...
class ExampleSwitch13(app_manager.RyuApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        print ("sniffer ip address= %r" % ip_dst)
    ...
    def _handle_arp(self, datapath, in_port, eth_pkt, arp_pkt):
        ...
        print (in_port, arp_src_ip, arp_dst_ip, arp_src_mac)
        ...
        if arp_dst_ip == HOST_IP:
            # create ethernet
            pkt = packet.Packet()
            pkt.add_protocol(ethernet.ethernet(ethertype= ether.ETH_TYPE_ARP, # ether.ETH_TYPE_8021Q,
                                               dst = arp_src_mac,
                                               src = HOST_MAC))
            # add vlan tag
