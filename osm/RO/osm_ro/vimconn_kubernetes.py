...
# replaced by self.url and self.user
kubernetes_ip = '206.189.248.107'   # IP of the machine running kubernetes
kubernetes_user = 'user'            # Name of the user-account that runs kubernetes



kubernetes_local_api_connector = '~/kubernetes_local_api_connector/kubernetes_local_api_connector.py' # location (and filename) of the local API connector for kubernetes
yaml_temp_folder = '~/kubernetes_local_api_connector/' # folder to temporarily store YAML files for creating pods etc. on the kubernetes-server
pod_template = ordereddict([('apiVersion', 'v1'), ('kind', 'Pod'), ('metadata', ordereddict([('name', 'nginx')])), ('spec', ordereddict([('containers', [ordereddict([('name', 'nginx'), ('image', 'nginx:1.7.9'), ('ports', [ordereddict([('containerPort', 80)])])])])]))])
...
class vimconnector():
    ...
    def new_vminstance(self, name, description, start, image_id, flavor_id, net_list, cloud_config=None, disk_list=None,
        ...
        if start != True:
            raise vimconnNotSupportedException("Kubernetes Pods cannot be paused")
        elif flavor_id is not None:
            raise vimconnNotImplemented("Flavors are not implemented") # TODO: implement flavors (hint: https://kubernetes.io/docs/tasks/configure-pod-container/assign-cpu-resource/)
        elif net_list is not None:
            raise vimconnNotImplemented("Custom networking is not implemented") # TODO: implement networking
            ...
            raise vimconnNotImplemented("'Availability zones list' parameter has no purpose for Kubernetes") # TODO: implement availability zones



        ########### The following is replaced by using V1 Objects instead if scp'ing a pod.yaml file ###########
        ######################################### It can be removed ############################################
        # pod = copy.deepcopy(pod_template) # use base template

        # # change attributes according to parameters
        # # TODO: implement more parameters
        # pod['metadata']['name'] = name
        # pod['spec']['containers'][0]['name'] = name
        # pod['spec']['containers'][0]['image'] = image_id

        # # write descriptor to file, copy to kubernetes-server and create pod
        # f = open('pod.yaml', 'w')
        # yaml.dump(pod, f)
        # f.close()

        # # TODO: This line assumes a pre-deployed ssh-key on the kubernetes-server. Room for improvement.
        # os.system('scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null pod.yaml {0}@{1}:{2} > /dev/null 2> /dev/null'.format(self.user, self.url, yaml_temp_folder))

        # std_out = ssh_run(ip=self.url, user=self.user, password=self.passwd, cmd='python {0} create-pod {1}/pod.yaml'.format(kubernetes_local_api_connector, yaml_temp_folder))


        pod_name = name
        pod_image = image_id
        
        # Map availability zone (VM) to namespace (Pod)
        # IMPORTANT: Any other namespace than 'default' will generate the following error:
        ...
        if availability_zone_index != 'default':
            raise Exception
        namespace_name = availability_zone_index
        
        std_out = ssh_run(ip=self.url, user=self.user, password=self.passwd, cmd='python {0} create-pod {1} {2} {3}'.format(kubernetes_local_api_connector, pod_name, pod_image, namespace_name))

        return tuple([std_out.replace('\n', ''), None])


    def get_vminstance(self, vm_id):
        """
        Returns the VM instance information from VIM
        :param vm_id: UID of the Pod
        :return: None or dictionary containing uid, name, ip, status
        """

        std_out = ssh_run(ip=self.url, user=self.user, password=self.passwd, cmd='python {0} get-pods'.format(kubernetes_local_api_connector))

        pod = {}
        response = json.loads(std_out)
        for i in range(len(response)):
            if response[i]['uid'] == vm_id:
                # print response[i]['uid'] + '\t' + response[i]['name'] + '\t' + response[i]['ip'] + '\t' + response[i]['status'] + '\t' + response[i]['ip'] + '\t' + response[i]['namespace']
                pod['uid'] = response[i]['uid']
                pod['name'] = response[i]['name']
                pod['ip'] = response[i]['ip']
                pod['status'] = response[i]['status']
    ...
    def delete_vminstance(self, vm_id, created_items=None):
        ...
        std_out = ssh_run(ip=self.url, user=self.user, password=self.passwd, cmd='python {0} delete-pod {1}'.format(kubernetes_local_api_connector, vm_id))
